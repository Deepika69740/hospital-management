// import axios from 'axios';

// const API = axios.create({
//   baseURL: 'http://localhost:5000/api', // Adjust based on your backend
// });

// // Hospital endpoints
// export const registerHospital = (hospitalData) => API.post('/hospitals', hospitalData);
// export const getHospitals = () => API.get('/hospitals');

// // Doctor endpoints
// export const registerDoctor = (doctorData) => API.post('/doctors', doctorData);
// export const getDoctors = () => API.get('/doctors');
// export const setDoctorAvailability = (doctorId, availability) => API.post(`/doctors/${doctorId}/availability`, availability);

// // Patient endpoints
// export const registerPatient = (patientData) => API.post('/patients', patientData);
// export const getPatients = () => API.get('/patients');

// // Appointment endpoints
// export const bookAppointment = (appointmentData) => API.post('/appointments', appointmentData);
// export const getAppointments = (userId, role) => API.get(`/appointments?userId=${userId}&role=${role}`);

// export default API;

import axios from 'axios';

const API = axios.create({
  baseURL: 'http://localhost:3001' // json-server port
});

// Hospital endpoints
export const getHospitals = () => API.get('/hospitals');
export const registerHospital = (data) => API.post('/hospitals', data);

// Doctor endpoints
export const getDoctors = () => API.get('/doctors?_embed=availability');
export const registerDoctor = (data) => API.post('/doctors', data);
export const setDoctorAvailability = (doctorId, availability) => 
  API.post(`/availability`, {...availability, doctorId});

// Patient endpoints
export const getPatients = () => API.get('/patients');
export const registerPatient = (data) => API.post('/patients', data);

// Appointment endpoints
export const bookAppointment = (data) => API.post('/appointments', data);
export const getAppointments = (userId, role) => {
  const key = role === 'doctor' ? 'doctorId' : 
             role === 'patient' ? 'patientId' : 
             'hospitalId';
  return API.get(`/appointments?${key}=${userId}&_expand=doctor&_expand=patient&_expand=hospital`);
};